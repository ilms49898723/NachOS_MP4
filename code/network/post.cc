...
PostOfficeInput::PostOfficeInput(int nBoxes) {
    static char msgAvail[50] = "message available";
    messageAvailable = new Semaphore(msgAvail, 0);
    ...
    static char postalWorkerString[50] = "postal worker";
    Thread* t = new Thread(postalWorkerString, 1);
...
PostOfficeOutput::PostOfficeOutput(double reliability) {
    static char messageSentString[20] = "message sent";
    static char messageSendLockString[20] = "message send lock";
    messageSent = new Semaphore(messageSentString, 0);
    sendLock = new Lock(messageSendLockString);
