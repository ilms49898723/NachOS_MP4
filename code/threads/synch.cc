...
void
Semaphore::SelfTest() {
    static char pingString[50] = "ping";
    Thread* helper = new Thread(pingString, 1);

    ASSERT(value == 0);     // otherwise test won't work!
    ping = new Semaphore(pingString, 0);
...
Lock::Lock(char* debugName) {
    name = debugName;
    static char lockString[10] = "lock";
    semaphore = new Semaphore(lockString, 1);  // initially, unlocked
    lockHolder = NULL;
}
...
void Condition::Wait(Lock* conditionLock) {
    ...
    static char conditionString[20] = "condition";
    waiter = new Semaphore(conditionString, 0);
